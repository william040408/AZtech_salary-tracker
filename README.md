...
1. **수집** — Gmail 에 오는 이카운트 전자문서 알림에서 링크를 찾아, 실제 브라우저로 열어 명세서 표를 읽어온다.
2. **검산** — 지급·공제 합계, 고용보험 0.9%, 주민세 10%, 실지급액을 다시 계산해 맞는지 본다.
3. **기록** — 쉰 날의 성격(연차·반차·무급·전사휴무·대체휴무)을 휴대폰 달력에서 직접 찍는다.
4. **대조** — 법정 계산값, 명세서에서 읽은 값, 직접 찍은 값을 세 열로 놓고 어긋나는 곳을 짚는다.

근태는 웹에서 직접 기록한다. 회사 근무현황 엑셀은 초기 기록을 한 번 들여올 때만
썼고(`config.json` 의 `workbook` 에 경로를 넣으면 다시 읽는다), 그 결과는
`data/attendance.json` 에 남아 있다.
